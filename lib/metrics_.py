...
def masked_mae_loss(scaler, null_val):
    def loss(preds, labels, feat_mean_values_np ,use_dtm=True):
        ...
        dtms_np = distance_to_mean_loss_vector(labels, feat_mean_values_np)
        dtm_weights = tf.convert_to_tensor(dtms_np, dtype='float32')
...
def masked_peak_mape_tf(pred, label, null_val, dtm_above_thresh):
    pass
...
def calculate_peak_metrics_tf(pred, label, null_val, feat_mean_values, max_value, DTM_TH):
    ...
    # either tf.math.divide or tf.truediv (?)
    dtm_current = tf.math.divide(dtm_current, max_value)
    dtm_above_thresh = tf.math.greater_equal(dtm_current, DTM_TH) * 1
